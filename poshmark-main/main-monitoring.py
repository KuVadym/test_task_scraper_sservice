...
async def listing(proxy: dict| str,  cooka: str, data: dict, user_id):
...
def parse_json(json_file: str):
    ...
    return proxy, cookies_path_, data, mode
...
async def gather_offers(proxy: dict| str,  cooka: str, user_id, data: dict = None):
    ...
    try:
        ...
        api = APIConnector(base_url="http://fastapi_app:8093")
        # with open('test_monitoring.json', 'w') as file:
        #     file.write(json.dumps(res))
        data = []
        for item in res:
            data += item
        print(f"Result: {data}")
        api.create_offers_bulk(data)
...
async def main_monitoring_offers():
    #while True:
    api = APIConnector(base_url="http://fastapi_app:8093")
    users = api.get_all_users_by_market(store_name="poshmark")
    for user in users:
        proxy = user.proxy
        ...
        print(proxy)
        if mode == 'monitoring':
            await gather_sales(proxy, 'cookies.json', user_id)
        else:
...
if __name__ == '__main__':
    ...
    asyncio.run(main_monitoring_offers())
